...
for dc in depth_categories:
    print(f"{dc}")
    tomo_df = pd.read_csv(Path(NI_tomo_dir) / "tomography_model_{}.xyz".format(dc), sep="\s", skiprows=4, header=None,
                          names=['X', 'Y', 'Z', 'Vp', 'Vs', 'rho', 'Qp', 'Qs'])

    tomo_df['lat'], tomo_df['lon'] = utm.to_latlon(np.array(tomo_df['X']),np.array(tomo_df['Y']),60,'G') #returns lat and lon
    ...
    for elev in z_values:
        print(f"    {elev}")
        crude_lats = tomo_df[tomo_df.Z==elev].lat # can be looped over mantle_depth_categories
        ...
        for V_type in V_types:
            ...
            Ti2=griddata(np.array([crude_lons,crude_lats]).T, np.array(T), (mgrid_lon,mgrid_lat), method='linear',fill_value=0) #outside the domain is 0
            write_file(step2_outdir, V_type, Ti2/1000, elev/1000, lats,lons)
            plt.imshow(Ti2,interpolation="nearest")
