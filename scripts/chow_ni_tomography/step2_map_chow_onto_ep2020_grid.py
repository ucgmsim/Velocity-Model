...
def process_per_elev(tomo_df, mgrid_lon, mgrid_lat, elev):
    ...
    for V_type in V_types:
        ...
        write_file(step2_outdir, V_type.lower(), Ti2, elev, lats, lons)
...
for dc in depth_categories:  # starting from shallow (higher res)
    print(f"{dc}")
    tomo_df = xr.open_dataset(
        Path(NI_tomo_dir / f"nz_atom_north_chow_etal_2021_vp+vs-{dc}.r0.1-n4.nc")
    )
    ...
    with Pool(8) as pool:
        res = pool.map_async(
            partial(process_per_elev, tomo_df, mgrid_lon, mgrid_lat), z_values
        )
        res = res.get()
