...
from shared import lats, lons, step1_outdir, step3_outdir, v_types, v_file_template, compare_dir


def compare(v_type, elev):
    final_array=pd.read_csv(step3_outdir/v_file_template.format(v_type,elev), delimiter=' ', header=2, na_filter=False).to_numpy()
    nzvm_array=pd.read_csv(step1_outdir/v_file_template.format(v_type,elev), delimiter=' ', header=2, na_filter=False).to_numpy()
    bias_array=np.log10(final_array/nzvm_array) # 0 if equal
    return bias_array


for v_type in v_types:
    for elev in [-23, -15, -5]: # to compare with the Chow paper's plots (25km is not available, so using 23km instead)
        ...
        df=pd.DataFrame({'lon':meshed_lons,'lat':meshed_lats,'bias':meshed_bias_array})
        df.to_csv(compare_dir/outfile, header=False,index=False,float_format="%.6f",sep=" ")
